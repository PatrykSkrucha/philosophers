...
{
	(*main)->num_of_philos = ft_atol(argv[0]);
	if (ft_atol(argv[0]) > 200)
	{
		print_error("Pass a number of philos from 1 to 200");
		return (1);
	}
	if (check_input(argv))
		return (1);
	(*main)->time_to_die = ft_atol(argv[1]);
	(*main)->time_to_eat = ft_atol(argv[2]);
	(*main)->time_to_sleep = ft_atol(argv[3]);
...
static int	limited_meals(char **argv, t_main **main)
{
	...
	(*main)->time_to_eat = ft_atol(argv[2]);
	(*main)->time_to_sleep = ft_atol(argv[3]);
	(*main)->number_of_meals = ft_atol(argv[4]);
